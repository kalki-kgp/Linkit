...
- On app open, resume, or Android network change, Android runs a 5-second Bonjour query filtered by the paired Mac's name, verifies the candidate with Mac identity proof, then updates the stored IP/port before re-registering its receiver.
...
Both sides converge on the same connection state. The Mac probes Android every ~30 s with a signed `GET /v1/devices/self/status`, and also forces a probe when macOS reports a network path change. Android refreshes its Mac registration in the background and, after ~45 s of silence, actively proves the Mac endpoint before marking it offline.
