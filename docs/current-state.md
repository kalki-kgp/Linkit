...
- Compose **Home** (control surface, kept short): Linkit wordmark (7-tap debug), device card (accent-gradient laptop avatar, name, pulsing status, reconnect), a **Feature status** card (this phone + the Mac's self-reported health; a broken feature shows a red dot and a tappable **Fix** that runs the resolve action inline — notification-listener rebind / permission request / battery-exemption / receiver-restart, all without leaving Home), and the action grid (send file, send clipboard, open link) + clipboard-sync toggle. Recent activity and the phone/notification/background config moved off Home into the Activity and Settings tabs.
