...
- Compose **Home** (control surface, kept short): Linkit wordmark (7-tap debug), device card (accent-gradient laptop avatar, name, pulsing status, reconnect), a feature **attention banner** (deep-links to Settings → Device), action grid (send file, send clipboard, open link) + clipboard-sync toggle, and a compact **Call mirroring** row that taps through to Settings → Phone. Recent activity and the phone/notification/background config moved off Home into the Activity and Settings tabs.
...
- **Settings** tab = **hub → detail** (`SettingsRoute`): a short hub of categories (Device, Clipboard, Notifications, Phone, Appearance, Background & battery, Updates, About) that each drill into a focused detail screen with a back arrow + large header (`SettingsDetailScaffold`) — mirroring the Mac Settings sidebar sections instead of one long scroll. The Device hub row shows an attention badge when a feature needs attention. **Device** detail = connection (status, address, reconnect/disconnect, pair with a different Mac, forget) + cross-device feature-status health (this phone + the Mac). **Appearance** = accent color (9 preset swatches + custom `#RRGGBB`, default `#D16B1F`) and theme (System/Light/Dark).
